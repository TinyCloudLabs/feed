...
function DataTrail({ card }: { card: FeedCard }) {
  const { quotes, notes, producer, sources, mediaKeys, summaryBits } = buildDataTrail(card);
  ...
  return (
    <details className="data-trail">
      ...
      <div className="data-trail-body">
        {producer.run.length > 0 && (
          <div className="data-row">
            <span>Run</span>
            <p>{producer.run.join(" · ")}</p>
          </div>
        )}
        {producer.delegation.length > 0 && (
          <div className="data-row">
            <span>Delegation</span>
            <p>{producer.delegation.join(" · ")}</p>
          </div>
        )}
        <div className="data-row">
          <span>TinyCloud</span>
          <p>
            artifact={card.id} · slug={card.slug} · schema=v{card.schema_version}
            {card.publisher_did ? ` · publisher=${card.publisher_did}` : ""}
            {mediaKeys.length > 0 ? ` · ${mediaKeys.join(" · ")}` : ""}
          </p>
        </div>
        <div className="data-row">
          <span>Checks</span>
          <p>
            {card.critic_pass ? "critic passed" : "critic failed"} ·{" "}
            {card.quotes_verified ? "quotes verified" : "quotes not verified"}
            {card.generation_model ? ` · ${card.generation_model}` : ""}
            {card.approval_status ? ` · approval=${card.approval_status}` : ""}
        ...
        {quotes.length > 0 && (
          <div className="data-row">
            <span>Quotes</span>
            <ul className="data-quotes">
              {quotes.map((q, i) => (
                <li key={`${q.quote}-${i}`}>
                  <q>{q.quote}</q>
                  {(q.speaker || q.transcript) && (
                    <cite>
                      {q.speaker ?? "Unknown"}
                      {q.transcript ? ` · ${fileLeaf(q.transcript)}` : ""}
                    </cite>
                  )}
                </li>
              ))}
