...
describe("Feed Host server", () => {
  test("serves seeded projections and records feedback/control intents through delegated actor storage", async () => {
    ...
    const artifact = await getJson<{ title: string; body: { text: string } }>(
      `${runtime.url}/artifacts/${encodeURIComponent(SEEDED_ARTIFACT_ID)}`,
      { "x-feed-actor-id": ACTOR_ID },
    );
    expect(artifact.title).toBe("Reviewed stub artifact");
    expect(artifact.body.text).toContain("grounded stub artifact");
    ...
    expect(eventsText).toContain(SEEDED_ARTIFACT_ID);
...
class FakeFeedHostStorage {
  ...
  async listFeedEvents(_actor: FeedHostActorStorage, afterEventId?: string): Promise<string> {
    ...
    const filtered = !trimmed
      ? events
      : events.slice(Math.max(0, events.findIndex((event) => event.id === trimmed) + 1));
