...
export class FeedHostStorage {
  ...
  async listFeedEvents(actor: FeedHostActorStorage, afterEventId?: string): Promise<string> {
    const projections = await this.listFeed(actor, { limit: 100 });
    return renderFeedEventStream(filterFeedEventsAfterId(buildFeedEvents(projections.items), afterEventId));
  }
...
function buildFeedEvents(projections: readonly FeedArtifactProjection[]): FeedSseEvent[] {
  const events: FeedSseEvent[] = [];
...
function filterFeedEventsAfterId(events: readonly FeedSseEvent[], afterEventId?: string): FeedSseEvent[] {
  const trimmed = afterEventId?.trim();
  if (!trimmed) return [...events];
  const index = events.findIndex((event) => event.id === trimmed);
  if (index < 0) return [...events];
  return events.slice(index + 1);
}

function renderFeedEventStream(events: readonly FeedSseEvent[]): string {
